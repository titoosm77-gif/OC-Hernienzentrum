...
(function() {
  ...
  function renderHerniaTypes() {
    ...
    const HAS_DETAIL = { leistenhernie: 1, nabelhernie: 1, narbenhernie: 1 };
    grid.innerHTML = window.HERNIA_TYPES.map(h => `
      <article class="hernia-card" data-type="${h.id}">
        ...
        ${HAS_DETAIL[h.id] ? `
          <button class="more-info-btn" data-detail="${h.id}">
            <span>${t('more_info_btn')}</span>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Hernia types: open relevant FAQ category, OR detail page if "more info" was clicked
    document.addEventListener('click', (e) => {
      // Priority 1: "Mehr Info" button -> jump to detail page
      const moreBtn = e.target.closest('.more-info-btn');
      if (moreBtn) {
        e.stopPropagation();
        const id = moreBtn.dataset.detail;
        showPage(id + '-detail');
        window.scrollTo({ top: 0, behavior: 'smooth' });
        return;
      }
